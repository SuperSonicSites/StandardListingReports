...
// interpretation live with the same rules /api/snapshot freezes.
import {
  EMPTY_MARKET_VALUES,
  interpretMarket,
  interpretProperty,
  ...
  function refreshPreview() {
    ...
    renderList(
      ...
      interpretProperty(
        values,
        ctx,
        { days_on_market: num("days_on_market") ?? 0, realtor_views: num("realtor_listing_views") ?? 0, showings: num("showings") },
        null
      )
    );
  }
