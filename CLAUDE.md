...
npm run check:site-order   # listing website orders against a fake Stripe + Zoho + Resend (no network)
npm run check:market       # Seller Market Update board parsers + rules against saved releases (no network)
```
...
### Listing websites (Stripe + Zoho CRM)

**Ships dark:** until `LISTING_SITES_ENABLED=1` is set on the server (with Stripe configured), the portal shows the card as "Coming soon" and the order page and `/api/site-order` refuse; `listingSitesEnabled()` in [src/lib/stripe.ts](src/lib/stripe.ts) is the single gate. `/c/<slug>/site` ([src/pages/c/[clientSlug]/site.astro](src/pages/c/[clientSlug]/site.astro), behavior in [src/scripts/site-order-form.ts](src/scripts/site-order-form.ts)) sells one **micro-site** (one route on the client's portfolio host, `portfolio.<clientdomain>/<route>`, holding one listing or a whole development as rows). Shared rules live in [src/lib/site-orders.ts](src/lib/site-orders.ts). The portal shows the card only when the profile has `zoho_account_id`, `portfolio_host` and Stripe is configured. Flow: `/api/site-order` ([src/pages/api/site-order.ts](src/pages/api/site-order.ts)) validates, reserves the route under a per-client lock (protected paths, uniqueness among live drafts and paid orders, drafts expire with their Checkout session), writes `data/orders/<id>.json` as `draft`, and creates a Stripe Checkout Session in subscription mode against the client's **existing** Stripe Customer (`stripe_customer_id` on the profile; created once only when blank) with the $599 one-time production price and the $99/year hosting price on a **365-day trial** (first year included), Stripe Tax on, the terms version+hash in the session metadata. Prices are found by lookup key (`listing_website_production_cad`, `listing_website_hosting_cad`), never by id. Payment is confirmed only by `fulfil()` in [src/lib/site-order-fulfil.ts](src/lib/site-order-fulfil.ts), called by the webhook (`/api/stripe-webhook`, signature-verified, open in the middleware), by the order page on return (`/c/<slug>/orders/<id>?session_id=…`) and by the admin Retry button; it runs under a per-order in-process lock and walks a persisted step ladder (verify session → record paid → `Listing_Websites` CRM record with the `Listings` subform and the first ChatGPT station prompt pre-filled in `Next_Step` (later prompts are set by the CRM Blueprint), `DUPLICATE_DATA` on the unique `Order_ID` recovers the existing id → confirmation email). Hosting state is derived from the fetched subscription on every invoice/subscription event, never incremented. Production stages live in the CRM Blueprint; the app never writes `Stage` after creation. Terms are versioned text in [src/lib/terms-listing-websites.ts](src/lib/terms-listing-websites.ts) (bump `TERMS_VERSION` when the text changes). Env: `STRIPE_SECRET_KEY` (restricted key), `STRIPE_WEBHOOK_SECRET`. Plan and decisions: `docs/listing-service/production-plan-v3.3.md`.

### Seller Market Update

The report form in market mode (`/c/<slug>/?type=market`, portal card "Seller Market Update"): the same pull plus this month's board statistics, for the seller asking "why isn't it selling?". Spec and decisions: [docs/seller-market-update-prd.md](docs/seller-market-update-prd.md). Pieces:

- [src/lib/market.ts](src/lib/market.ts) — the `MARKETS` table (Central Okanagan, Vancouver Island board-wide, Tofino-Ucluelet with live local inventory from VIREB's public listings site, Yukon), fetchers and one parser per board, the monthly cache under `data/market/`, the on-demand refresh with a 24 h attempt throttle, and the exposure benchmark ledger. Sources are undocumented: the Interior REALTORS dashboard is a WordPress admin-ajax action guarded by a nonce read from their public page (year-over-year comes from fetching the same month a year earlier); CREA Stats `page-data.json` is transport only and carries each board's own prose (VIREB writes percentages under ten as words; VIREB publishes no days to sell). When a source changes, the pull degrades to a warning and the coordinator types the numbers.
- [src/lib/market-rules.ts](src/lib/market-rules.ts) — browser-safe thresholds and the deterministic interpretation sentences (small-market guardrail under 30 sales; the exposure comparison against our own archive only once the ledger holds 30 qualifying reports). The form previews them live ([src/scripts/market-form.ts](src/scripts/market-form.ts)); `/api/snapshot` recomputes them from the submitted values and freezes them. They never recommend a price change.
- Client profile `market` (admin-set select), form `property_type`, snapshot `report.kind` + optional `market` block; the report gains one market sheet and, for a market update, drops the social post sheet (social views stay as numbers — owner decision). A market update leads with "Where your property stands": a cover verdict (`sellerVerdict`, frozen as `report.verdict`) and three proof numbers, then Your listing → Market → Compliance. It stays one click — the agent types nothing (owner decision). Admin dashboard "Refresh market data" → `/api/market-refresh` force-fetches every market and overwrites the cache; snapshots never change.
- Region labels carry no article (they open sheet headings and follow "in" in sentences).

### Storage layer

All disk I/O goes through [src/lib/storage.ts](src/lib/storage.ts) — never read/write `data/` files directly elsewhere. Four stores, all plain JSON files, written atomically (temp file + rename):
...
- `data/orders/<id>.json` — listing website orders (intake, agreement evidence, payment and fulfilment steps, history). Stripe and the CRM record are the records of record; this file is the app's cache and can be rebuilt from them.
- `data/market/<market>-<YYYY-MM>.json` — cached monthly board statistics (**gitignored** except `.gitkeep`), plus a per-market attempt marker and the exposure ledger. Replaceable; snapshots copy what they need.
